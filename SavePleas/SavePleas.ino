...
#define SERVO_SHOULDER1_CHANNEL 4
#define SERVO_SHOULDER2_CHANNEL 5

// Current servo angles for smooth movement
int flexSensorHandAngle = 0;
int flexSensorWristAngle = 0;
int flexSensorElbowAngle = 0;
int GYROUpperElbowAngle = 0;
int GYRORollShoulder1Angle = 90;
int GYRORollShoulder2Angle = 90;
...
void loop() {
  ...
  // 1. Calculate raw accelerometer roll (degrees)
  accelRoll = atan2(ay, az) * 180.0 / PI;

  // 2. Get gyro X-axis data (convert to degrees/sec)
  gyroX = gx / 131.0;

  // 3. Complementary filter: Combine gyro + accelerometer
  ...
  int targetElbowAngle = map(flexSensorElbowValue, 600, 2000, 30, 150);
  ...
  int targetShoulder1Angle = map(roll, -45, 45, 60, 120);  // Adjust range as needed
  int targetShoulder2Angle = map(roll, -45, 45, 120, 60);  // Inverse movement
  ...
  targetShoulder1Angle = constrain(targetShoulder1Angle, 0, 180);
  targetShoulder2Angle = constrain(targetShoulder2Angle, 0, 180);
  ...
  GYRORollShoulder1Angle += constrain(targetShoulder1Angle - GYRORollShoulder1Angle, -stepSize, stepSize);
  GYRORollShoulder2Angle += constrain(targetShoulder2Angle - GYRORollShoulder2Angle, -stepSize, stepSize);
  ...
  GYRORollShoulder1Angle = constrain(GYRORollShoulder1Angle, 0, 180);
  GYRORollShoulder2Angle = constrain(GYRORollShoulder2Angle, 0, 180);
  ...
  pwm.setPWM(SERVO_SHOULDER1_CHANNEL, 0, angleToPulse(GYRORollShoulder1Angle));
  pwm.setPWM(SERVO_SHOULDER2_CHANNEL, 0, angleToPulse(GYRORollShoulder2Angle));
